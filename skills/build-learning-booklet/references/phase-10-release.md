...
Produces: an engine-derived release decision report (optionally written as `release-manifest.json`), final reports, maintenance notes, and terminal run decision. Repository archive construction and OSS packaging checks are separate release-infrastructure steps.

## Acceptance

Run `verify-release.mjs --run <run-root-or-run-state.json>`, optionally with `--report <release-manifest.json>`. The wrapper loads canonical state, submits `release.decide` through the workflow engine, validates the event log, and reports the engine's deterministic decision. It does not scan the plugin tree, create an archive, calculate distribution checksums, or perform license/secret checks.
...
- both `native-macos-intel` and `native-macos-apple-silicon` have current passing evidence for the exact candidate when plugin release is in scope.
